...
- [x] **Lombok 纳入 base 作能力基座**：`version.lombok=1.18.46`（中央仓库最新线，实测字节码 version 50.0 即 Java 1.6），dependencyManagement 钉版 + 顶层 dependencies 无条件注入、scope=provided（编译期注解处理器，不进运行期、不向下游传递）；spring-boot-dependencies 的 lombok 管理仅钉版（1.18.30，无 scope/optional），按优先级规则 base 直接钉版胜出——临时项目实测继承 boot（spring）与继承 base（非 spring）两条线均为 1.18.46 provided，有利结果：1.18.30 在 JDK 21+ 跑不动。
