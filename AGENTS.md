...
| `boot` | 存在 `src/main/java` 时激活 `spring-boot` Profile，用 git-commit-id-plugin 生成 git.properties（含 `git.commit.id.describe` 版本信息，spring actuator `/info` 可直接展示）、用 spring-boot-maven-plugin（2.7.7）repackage 可执行 jar |
